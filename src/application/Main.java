...
public class Main extends Application {
	...
	public void start(Stage stage) {
		try {
			...
			//Tries to loading image file from file path
			int isSucc = loadImageFile("//userfs/ce715/w2k/Desktop/0.jpg");
	...
	public void runProgram(){
		...
		imgPro.getImageContainer().setOnMouseDragged(e->{
			//Check if the cursor position is with in the scenes bounds
			if ((e.getSceneX() > 0 && e.getSceneX() < scene.getWidth())
					&&(e.getSceneY() > 0 && e.getSceneY() < scene.getHeight())){
				//If cursor within width and height bounds, move image to cursors position
	...
	/**
	 * Attempts to load image file from specified file path 
	...
	public int loadImageFile(String filePath){
		...
		//Specific the image file to be used
		File imgFile = new File(filePath);
		//Stores the imgFile files extension from the file path;
		...
		//If file path and file type are valid, loaded the image file and return 0
