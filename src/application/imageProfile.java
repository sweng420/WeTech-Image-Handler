...
public class imageProfile {
	...
	public imageProfile(Image image){
		...
		//Store the ration of the height of width of the original image
		this.imageRatio = image.getHeight()/image.getWidth();
		
		//Create a new ImageView node
		this.imageNode =  new ImageView(image);
		//Preserves the width and height ratio when either value is changed
		...
		//Calculates and stores the ImageView node height after default scaling
	...
	/**
	 * Gets the value of the HBox container
	...
	public void setPosition(double xPos, double yPos, Scene scene){
		...
		//Checks if new x position is within the scenes width bounds
		if (xPos > 0 && xPos < scene.getWidth()){
			this.imageContainer.setLayoutX(xPos -(this.imageFit/2));
		}
		//Checks if new y position is within the scenes width bounds
	...
	/**
	 * Increases or decreases the image size by a specified increment 
	 * @param delta value for the image to be scaled with. Can be positive or negative.
	 */
	public void scaleImage(double delta){
		
		//Store the image fits value once scaled by the values delta
	...
	/**
	 * Resets the position and size of the image to the default values
